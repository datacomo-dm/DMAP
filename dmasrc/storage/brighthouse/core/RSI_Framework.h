...
public:
	...
	RSIndex* GetIndex( RSIndexID id, int read_loc ,const bool	from_truncate_partition= false);
	...
	RSIndex* GetIndexForUpdate(	RSIndexID id, int read_loc ,const bool	from_truncate_partition= false);
...
private:
	...
	RSIndex* LoadIndex(RSIndexID id, bool write, int read_loc,const bool	from_truncate_partition= false);		// when write=true, file is created if needed; return NULL if file can't be loaded (e.g. already locked)
...
private:
    ...
    bool from_truncate_partition;  // 从删除分区的分区上加载数据,add by liujs
...
public:
    ...
    void Set_FromTruncatePartition(const bool v){ from_truncate_partition = v;}
    bool Get_FromTruncatePartition(){ return from_truncate_partition;}
protected:
