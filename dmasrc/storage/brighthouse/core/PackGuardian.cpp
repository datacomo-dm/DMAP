...
void VCPackGuardian::PreloadPack(const MIIterator &mit,std::vector<_int64 *> &prefetches) {
	...
	for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++,fct++) {
		...
		if(tab != NULL){
			tab->PreloadPack(iter->col_ndx,prefetches[fct],my_vc.ConnInfo());
		}
	}
}
...
PG_Status VCPackGuardian::LockPackrow(const MIIterator &mit,std::vector<_int64 *> &prefetches,int threadt)
{
	if(!initialized)
		Initialize(mit,threads);
	int fsize=prefetches.size();
	if(current_strategy == LOCK_ONE) {
		...
		for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++,fct++) {
			...
			if(last_pack[cur_dim][threadt] != mit.GetCurPackrow(cur_dim)) {
				...
				if(tab != NULL){
					...
					try {	
						tab->LockPackForUse(iter->col_ndx,fsize>fct?prefetches[fct]:NULL, mit.GetCurPackrow(cur_dim), my_vc.ConnInfo());
					} catch(...) {
		...
		for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++,fct++) {
			if(pack_locked[iter->dim][mit.GetCurPackrow(iter->dim)] == 0) {
				try {
					iter->GetTabPtr()->LockPackForUse(iter->col_ndx,fsize>fct?prefetches[fct]:NULL, mit.GetCurPackrow(iter->dim), my_vc.ConnInfo());
				} catch(...) {
					// unlock packs which are partially locked for this packrow
					vector<VirtualColumn::VarMap>::const_iterator it = my_vc.GetVarMap().begin();
					for(; it != iter; ++it) {
