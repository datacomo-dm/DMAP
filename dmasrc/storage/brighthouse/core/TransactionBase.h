...
public:
    ...
    void ReleaseLocalObject(int table); // add by liujs ,删除local_cache内的包
    void ReleaseGlobalObject(int table); // add by liujs ,删除global_cache内的包
 
    virtual void SetTableReleaseFlag(std::string const& table_path,bool release_cache_pack)=0;
