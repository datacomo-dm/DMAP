...
void TempTable::LockPackForUse(unsigned attr, _int64 *pahead, unsigned pack_no, ConnectionInfo& conn)
{
	while(lazy && no_materialized < min(((_int64)pack_no << 16) + 0x10000, no_obj))
		Materialize(false, NULL, true);
}

void TempTable::PreloadPack(unsigned attr, _int64 *pahead,ConnectionInfo& conn)
{}
