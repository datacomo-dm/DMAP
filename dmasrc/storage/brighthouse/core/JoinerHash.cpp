...
void JoinerHash::ExecuteJoin()
{
	...
	if(dim1_size > 0 && dim2_size > 0)
		while(tr_mit.IsValid()) {
			...
			joined_tuples += (SHJOIN_MPP_N>0&&CanMatchDimMPP(new_mind))?MatchDimMPP(new_mind, match_mit):MatchDim(new_mind, match_mit);
			if(watch_traversed)
				outer_tuples +=  SubmitOuterTraversed(new_mind);

            if(SHJOIN_MPP_N>0&&CanMatchDimMPP(new_mind)){
			    rccontrol.lock(m_conn.GetThreadID()) << "MatchDimMPP Produced " << joined_tuples << " tuples." << unlock;
            }else{
                rccontrol.lock(m_conn.GetThreadID()) << "MatchDim Produced " << joined_tuples << " tuples." << unlock;
            }
