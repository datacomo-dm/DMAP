...
int AttrPackN::Save(IBStream* fcurfile, DomainInjectionManager& dim)
{
	...
	*((uint*) head) = total_size;
	...
	fcurfile->Write((char*) head, 17);
	if(IsModeNoCompression()) {
		if(no_nulls)
			fcurfile->Write((char*) nulls, (no_obj + 7) / 8);
		if(data_full)
			fcurfile->Write((char*) data_full, value_type * no_obj);
	} else
...
#if 0
	if (this->optimal_mode == 0) {
		do {
			msgIBRCAttrPack msgPack;
			std::string msgPack_str;
			{
				std::string msgPack_str_c((const char*)compressed_buf, comp_buf_size);
				if (CR_Class_NS::decompressx(msgPack_str_c, msgPack_str) != 0)
					break;
			}
			if (!msgPack.ParseFromString(msgPack_str))
				break;
			int pack_type = msgPack.pack_type();
			if ((pack_type != msgIBRCAttrPack::PACK_N)
			  &&(pack_type != msgIBRCAttrPack::PACK_F))
				break;
			{
				const std::string &pack_nulls = msgPack.nulls();
				if ((this->nulls == NULL) && (pack_nulls.size() > 0)) {
					this->nulls = (uint*)alloc(pack_nulls.size(), BLOCK_UNCOMPRESSED);
					memcpy(this->nulls, pack_nulls.data(), pack_nulls.size());
				}
			}
			{
				const std::string &pack_data = msgPack.data();
				if ((this->data_full == NULL) && (pack_data.size() > 0)) {
					this->data_full = alloc(pack_data.size(), BLOCK_UNCOMPRESSED);
					memcpy(this->data_full, pack_data.data(), pack_data.size());
				}
			}
			this->compressed_up_to_date = true;
			dealloc(this->compressed_buf);
			this->compressed_buf = NULL;
			this->comp_buf_size = 0;
			return;
		} while (0);
	}
#endif 
...
//	int last_val[]={-1,-1,-1,-1,-1,-1,-1,-1};
	...
	if(no_nulls > 0) {
		...
		if(no_obj < 65536 || null_buf_size<8192)
			memset(nulls, 0, 8192);
		if(null_buf_size > 8192)
		...
		if(!IsModeNullsCompressed() ) // no nulls compression
			...
			BitstreamCompressor bsc;
			CprsErr err=bsc.Decompress((char*) nulls, null_buf_size, (char*) cur_buf + 2, no_obj, no_nulls);
			// For tests:
