...
TransactionBase::GetAttrPack(const PackCoordinate& pack_coord, PackAllocator &a)
{
	...
	if (tv.table && (uint)(pc_dp( pack_coord ) + 1) >= tv.initial_no_packs) {
		ret = m_localCache.GetObject<AttrPack>(pack_coord, bind( &AttrPackLoad, boost::ref(pack_coord), boost::ref( a ) ));
	} else{
		ret = m_globalCache->GetObject<AttrPack>(pack_coord, bind( &AttrPackLoad, boost::ref(pack_coord), boost::ref( a ) ) );
	}
...
void TransactionBase::ReleaseLocalObject(int table) // add by liujs ,删除local_cache内的包
{
    m_localCache.ReleasePacks(table);
    m_localCache.ReleaseDPNs(table);    
} 


void TransactionBase::ReleaseGlobalObject(int table) // add by liujs ,删除global_cache内的包
{
    m_globalCache->ReleasePacks(table);
    m_globalCache->ReleaseDPNs(table);      
}
