	...
	try {
		...
		if(!IsModeValid()) {
			...
			if(IsModeNoCompression()) {
				...
				// normal compression
				index = 0;
				if((optimal_mode == 0) || IsModeCompressionApplied()) // 0,1	= Huffmann 16-bit compression of remainings with tree (0 - flat null encoding, 1 - compressed nulls)
				{
	...
	if (this->optimal_mode == 0) {
		do {
			...
			if (!msgPack.ParseFromString(msgPack_str))
				break;
			if (msgPack.pack_type() != msgIBRCAttrPack::PACK_S)
				break;
			memcpy(this->nulls, msgPack.nulls().data(), msgPack.nulls().size());
			memcpy(this->lens, msgPack.lens().data(), msgPack.lens().size());
			memcpy(*(this->data), msgPack.data().data(), msgPack.data().size());
			uchar *dp = *(this->data);
			for(size_t id=0; id< this->no_obj; id++) {
				if(!this->IsNull(id) && (this->GetSize(id) > 0)) {
					this->index[id] = dp;
					dp += this->GetSize(id);
				} else {
					this->SetSize(id, 0);
...
int AttrPackS::Save(IBStream* fcurfile, DomainInjectionManager& dim)
{
	...
	*((ushort*) (head + 7)) = (ushort) (no_nulls);
	...
	if(IsModeCompressionApplied()) {
		...
		*((uint*) head) = total_size;
		fcurfile->Write((char*) head, head_size);
		fcurfile->Write((char*) compressed_buf, comp_buf_size);
	} else if(IsModeNoCompression()) {
		SaveUncompressed(head, fcurfile);
	} else {
...
void AttrPackS::SaveUncompressed(uchar* head, IBStream* fcurfile)
{
	...
	*((uint*) head) = total_size;
	...
	if(data) {
		...
		fcurfile->Write(tmpbuf.get(), string_size);
	}

	if(nulls)
		fcurfile->Write((char*) nulls, (no_obj + 7) / 8);
	if(lens)
		fcurfile->Write((char*) lens, len_mode * no_obj);
}
