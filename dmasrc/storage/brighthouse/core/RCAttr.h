...
// Methods declared in PhysicalColumn.h or RCAttr-specific
	...
	virtual void	LockPackForUse(_int64 *pahead,unsigned pack_no, ConnectionInfo& conn);
	void 			LockPackForUse(_int64 *pahead,unsigned pack_no, Transaction& trans,boost::function0<void> notify_pack_load = boost::function0<void>());
	virtual void	LockPackForUse(unsigned pack_no);
	int LockPackForThread(unsigned pack_no, Transaction& trans);
	void DeliverPreLoad(_int64 *pahead,DMThreadGroup *pmtr,Transaction& trans,DMThreadData *pThreadData);
	void PreLoadPack(_int64 *pahead,ConnectionInfo& conn);
...
//	int ColumnSize()	 const;
    ...
    bool NeedToCreateRSIIndex();        // 是否需要创建rsi索引数据,fix dma-1443
...
//	bool VerifyRSI(); 					// Repairs index directory: false if the repair was needed and done.
	...
	uint packs_omitted;                 // 装入数据过程的过滤包数,只加载最后一部分数据包
	...
	std::string LongSessionCommitFile();
	int LoadLongSessionComitFile(std::string &partname,std::vector<int>& del_pack_file_lst);
	int SaveLongSessionComitFile(const char* partname,const std::vector<int> del_pack_file_lst);
	void ClearLongSessionCommitFile();
	void RestoreDPN(char* buf, uint pack_no);
	void RestoreDPN(char* buf, DPN& dpn, bool reset_pack =true);
...
protected:
    ...
    void StoreDPN(DPN dpn,char *buf); // 将dpn转换成buff,add by liujs
	void SaveRSI();
...
//	int				string_no_dict;
    ...
    bool            load_data_from_truncate_partition;         // 分布式排序合并数据,长回话合并时候需要从dpn.ntrunc,rsi.ntrunc文件中加载到内存后继续合并数据
    int             load_data_from_truncate_partition_packs;   // 记录下来删除分区后的包的数,重新加载dpn的时候用到
    
    bool            skip_DoSavePack_packidx;                   // 从mysqld中合并数据包过程,在DoSavePack中跳过包索引的合并过程,
                                                               // 最后再统一的进行合并包索引
                                                               
    int             merge_packidx_packno_init;                 // 跳过包索引的起始包号,分布式排序单次生成的所有的包索引的pack编号都是从0开始的
                                                               // 此处需要添加上上一次数据装入的包的数量
                                                               // 长回话上合并数据与load_data_from_truncate_partition_packs相同

	bool     commit_long_session_merge_data(); // 提交数据
	bool     prepare_commit_long_session_merge_data(); // 判断是否满足提交条件
...
public:
    bool    Get_LoadDataFromTruncatePartition(){ return load_data_from_truncate_partition;}
	void    Set_LoadDataFromTruncatePartition(bool v){ load_data_from_truncate_partition = v;}
    int     Get_LoadDataFromTruncatePartitionPacks(){ return load_data_from_truncate_partition_packs;}
	
    // 保存分布式排序合并过后的每一个列的数据头
    void save_merge_table_header();
	void SaveDPN();
private:
...
public:
	struct push_param {
		...
		RCAttr *pattr;
		Transaction* trans;
		push_param(_int64 *_pahead,RCAttr *_pattr,Transaction *_trans):
			pattr(_pattr),trans(_trans) {
...
private:
	...
	MultiThread_Preload() {
		...
		pusher=new DMThreadGroup(5,max_mtr);
	}
public :
	~MultiThread_Preload() {
		delete pmtr;
	...
	static void PushThread(DMThreadData *pdata) {
		...
		opts->pattr->DeliverPreLoad(opts->pahead, GetInstance()->pmtr,*opts->trans,pdata);
	}
	// push to thread list or push to buffer(async?)
	// return 0: empty preload data 1:has pushed preload data
	int PushPreload(_int64 *pahead,RCAttr *pattr,Transaction& trans) {
		if(pahead[0]-pahead[1]<1) return 0;
		push_param opts(pahead,pattr,&trans);
		//push 数据队列满,则放弃预读取
