...
public:
	...
	_int64 CurrentRow() {return current_row;}
	_int64 ToBeReturned() {return to_be_returned;}
	_int64 CurrentIterateStep() {return current_iterate_step;}
	int KeyBufWidth() {return key_buf_width;}
	void PutMatchedValue(unsigned char*	ibuffer,int col, VirtualColumn *vc, MIIterator &mit)			// for all values EXCEPT NULLS
	{
		encoder[col].Encode(ibuffer, mit, vc);
	}
	void PutMatchedValue(unsigned char*	ibuffer,int col, _int64 v)			// for constants
	{
		encoder[col].PutValue64(ibuffer, v, true, false);		// true: the second encoded column false: don't update stats
	}

	_int64 InitCurrentRowToGet(unsigned char* ibuffer,_int64 &_current_row,_int64 &_to_be_returned,_int64 &_current_iterate_step);
	_int64 GetNextRow(unsigned char* ibuffer,_int64 &_current_row,_int64 &_to_be_returned,_int64 &_current_iterate_step);
private:
